...
For this example, we would like to install the ```tidyverse``` and ```shiny``` in one line (note: we won't be using shiny in this tutorial). Recall that we can use the combine argument, ```c("packageOne", "packageTwo")```.
...
Great, if you now look at your console, you can see that we're given information about the classes of the dataset. It's always great to do a quick scan of this to see whether anything is weird. Just for fun, I tried using the `read.csv` function to read the Pokemon dataset, and I noticed that `read.csv` reads in data with numbers as an integer type, whereas the `read_csv` reads numbers as a numeric/double type. 
...
We can also use `group_by` with multiple arguments, such as grouping by the type of Pokémons and whether it's legendary or not. With the help of Google, I realized that there might be something special about legendary Psychic Pokémons. Let's see what's going on:
